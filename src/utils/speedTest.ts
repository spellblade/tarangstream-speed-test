...
export async function measurePing(
  ...
  if (
    userCoords &&
    server &&
    server.lat !== undefined &&
    server.lon !== undefined
  ) {
    ...
    if (server.id.includes("oregon") || server.id.includes("west"))
      baseLatency = 45;
    else if (server.id.includes("frankfurt") || server.id.includes("europe"))
      baseLatency = 110;
    else if (server.id.includes("singapore") || server.id.includes("asia"))
      baseLatency = 180;
    ...
    else if (server.id.includes("virginia") || server.id.includes("east"))
      baseLatency = 35;
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    if (server && server.url) {
      try {
        const cleanUrl = server.url.startsWith("http")
          ? server.url
          : `https://${server.url}`;
    ...
    if (!didRealProbe) {
      try {
        ...
        const noise =
          (Math.random() - 0.5) * (baseLatency * 0.12) +
          Math.sin(i) * (baseLatency * 0.04);
...
export function getSpeedCurveValue(
  ...
  const periodicNoise =
    Math.sin(elapsedPercent * 65) *
    Math.sin(elapsedPercent * 12) *
    interferenceRange;
  ...
  return new Promise<RealTestResult>((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      ...
      if (smoothedSpeedEMA === 0) {
        ...
        smoothedSpeedEMA =
          alpha * sliceSpeedMbps + (1 - alpha) * smoothedSpeedEMA;
      }
      ...
      if (rawSamples.length > 1) {
        ...
        variance =
          rawSamples.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
          rawSamples.length;
      }
      const varianceFactor = Math.min(
        0.02,
        (Math.sqrt(variance) / (smoothedSpeedEMA + 1)) * 0.1,
      );
      const estimatedPacketLoss = Math.min(
        8.5,
        parseFloat(((stallRatio * 4.5 + varianceFactor * 10) * 100).toFixed(2)),
      );
      ...
      if (elapsedMs >= durationMs) {
        ...
        const finalMeanEMA =
          trimmedSamples.reduce((sum, s) => sum + s, 0) / trimmedSamples.length;
...
export async function runRealUploadTest(
  ...
  return new Promise<RealTestResult>((resolve, reject) => {
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && !isFinished && !signal?.aborted) {
          ...
          if (xhr.status >= 400 || xhr.status === 0) {
            console.warn(
              `[Upload XHR Error] status=${xhr.status}. Triggering fail-safe simulation fallback.`,
            );
    ...
    const interval = setInterval(() => {
      ...
      const estimatedPacketLoss = Math.min(
        6.8,
        parseFloat(((stallRatio * 3.8 + varianceFactor * 8) * 100).toFixed(2)),
      );
