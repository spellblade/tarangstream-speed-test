...
export function getPublicDownloadTestUrl(server?: ServerOption): string {
  ...
  const key = Object.keys(PUBLIC_DL_MIRRORS).find(
    (k) => server.id.toLowerCase().includes(k) || loc.includes(k),
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      ...
      const progressPercent = Math.min(
        100,
        ((performance.now() - start) / durationMs) * 100,
      );
      ...
      const simEMA =
        lastSimEMA === 0 ? speed : alpha * speed + (1 - alpha) * lastSimEMA;
      lastSimEMA = simEMA;
      ...
      const simLoss = Math.min(
        6.8,
        parseFloat(
          Math.max(0, jitter / 200 + Math.sin(ticks / 6) * 0.25).toFixed(2),
        ),
      );
...
export {
  runLocalDownloadTest as runRealDownloadTestForLocal,
  runLocalUploadTest as runRealUploadTestForLocal,
};
