...
export default function Gauge({
  ...
  return (
    <div className={`relative flex flex-col items-center bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 border ${borderTheme} dark:border-slate-800/80 ${shadowTheme} min-w-[270px] flex-1 transition-all duration-300`}>
      {/* Top Banner Tag */}
      <div className="flex justify-between items-center w-full mb-5">
        ...
        <div className="flex items-center gap-1.5">
          <span className={`w-2 h-2 rounded-full ${isActive ? (isCyan ? 'bg-blue-500 animate-ping' : 'bg-emerald-500 animate-ping') : (isCompleted ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700')}`} />
      ...
      <div className="relative w-72 h-72 md:w-80 md:h-80 flex items-center justify-center">
        <svg className="w-full h-full select-none overflow-visible" viewBox="0 0 260 260">
          <defs>
            ...
            <filter id={`glowFilter-${colorClass}`} x="-20%" y="-20%" width="140%" height="140%">
          ...
          {ticks.map((tick, i) => {
            ...
            const tickAngleInRad = ((startAngle + (tickPercent * sweepAngle)) * Math.PI) / 180;
            ...
            return (
              <g key={i}>
                <line
                  ...
                  x2={endX}
                  y2={endY}
                  stroke={percent >= tickPercent ? (isCyan ? '#3b82f6' : '#10b981') : undefined}
                  className={percent >= tickPercent ? "" : "stroke-slate-200 dark:stroke-slate-800/80"}
                  strokeWidth="2"
                />
                <text
                  x={labelX}
      ...
      <div className="mt-4 w-full flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4 select-none">
        <div className="flex flex-col">
          <span className="text-xs uppercase font-mono text-slate-400 dark:text-slate-500 font-bold block">Peak Speed</span>
        ...
        <div className="flex flex-col items-end">
          <span className="text-xs uppercase font-mono text-slate-400 dark:text-slate-500 font-bold block">Gauge Dial</span>
          <span className={`text-sm font-mono font-semibold ${textTheme} mt-0.5 ${activeGlow}`}>
